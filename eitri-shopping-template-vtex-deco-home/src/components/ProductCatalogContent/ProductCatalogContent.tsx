...
export default function ProductCatalogContent(props: ProductCatalogContentProps) {
	...
	const getInitialParams = useCallback((): CatalogParams | null => {
	...
	const getProducts = async (selectedFacets: CatalogParams, page: number) => {
		try {
			...
			const result = (await getProductsService(selectedFacets, page)) as {
	...
	const onScrollEnd = async () => {
		if (!productLoading && !pagesHasEnded) {
			...
			if (appliedFacets) getProducts(appliedFacets, newPage)
		}
	}

	const handleSortChange = (newSort: string) => {
		const newParams: CatalogParams = {
			...appliedFacets,
			sort: newSort
		}
	...
	const onFilterClear = () => {
		...
		if (initialFilters) handleFilterChange(initialFilters)
	}
