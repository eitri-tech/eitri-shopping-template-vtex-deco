...
const getSegments = (
	...
	return sorted.slice(0, -1).reduce<Segment[]>((acc, start, i) => {
		...
		const entity = entityRange != null ? (entityMap?.[entityRange.key] ?? null) : null
...
function InlineText(props: InlineTextProps) {
	const { text, ranges, entityRanges, entityMap, base } = props
...
const renderBlock = (block: DraftBlock, index: number, blocks: DraftBlock[], entityMap?: Record<string, Entity>) => {
