...
export default function CategoryListVtex(props) {
	const { setPageTitle } = props
	const { data } = props
	const exclusionList = data?.exclusionList

	const [stack, setStack] = useState(cachedStack ?? [{ title: null, items: [] }])
	const [animating, setAnimating] = useState(false)
	const [direction, setDirection] = useState('forward') // forward | back
	const [isLoading, setIsLoading] = useState(false) // forward | back
	...
	const buildExclusionSet = raw => {
		if (!raw) return new Set()
		return new Set(raw.split(';').map(item => item.trim().toLowerCase()))
	}

	const filterCategories = (categories, exclusionSet) => {
