...
export default function Wishlist(props: WishlistProps) {
	...
	const [itemWishlistId, setItemWishlistId] = useState<string>('-1')
	...
	const handleSaveFavorite = async () => {
		if (itemWishlistId === '-1') {
			try {
				...
				)) as { data?: { addToList?: string } }
				setItemWishlistId(result?.data?.addToList ?? '-1')
			} catch (e) {
	...
	const checkIfIsFavorite = async (productId: string) => {
		setLoadingWishlist(true)
		const { inList, listId } = await productOnWishlist(productId)
		if (inList) {
			setItemWishlistId(listId ?? '-1')
			setItemOnWishlist(true)
		}
