...
interface ElasticDotsProps {
	total: number
	current: number
	maxVisible?: number
}

export default function ElasticDots(props: ElasticDotsProps) {
	const { total, current, maxVisible = 7 } = props

	// A bare `return` (no value) is invalid from a component's render — React expects an
	// element or null. Below 2 dots there's simply nothing to render.
	if (total < 2) {
		return null
	}
