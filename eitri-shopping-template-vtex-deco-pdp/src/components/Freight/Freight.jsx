...
import { CustomButton, CustomInput, CloseIcon } from 'eitri-shopping-template-vtex-deco-shared'
...
export default function Freight(props) {
	...
	const deliveryOptions = [...allOptions.filter(o => !o.isPickupInPoint)].sort((a, b) => {
		...
		return new Date(a.shippingEstimateDate) - new Date(b.shippingEstimateDate)
	})
	const pickupOptions = allOptions
		...
		.sort((a, b) => {
			...
			const hasDistanceA = a.pickupDistance !== null && a.pickupDistance !== undefined && Number.isFinite(distanceA)
			const hasDistanceB = b.pickupDistance !== null && b.pickupDistance !== undefined && Number.isFinite(distanceB)
	...
	return (
		<View className='flex flex-col gap-4 p-4 mt-8 w-full bg-[#E8E6DF]'>
			...
			{pristine && (
				<View onClick={() => Eitri.openBrowser({ url: 'https://buscacepinter.correios.com.br/app/endereco/index.php' })}>
					...
					{deliveryOptions.length > 0 && (
						<View
							...
							{deliveryOptions.map((option, index) => (
								<View key={option.id} className='flex flex-row gap-3'>
							...
							<View className='flex flex-row items-center justify-between'>
								...
								{pickupOptions.length > 1 && (
									<View
										...
										{showAllPickupOptions ? <FiMinus size={20} /> : <FiPlus size={20} />}
									</View>
								)}
							</View>
							{visiblePickupOptions.map(option => (
								<View key={option.id} className='flex flex-row gap-3'>
									<Text className='text-sm shrink-0 w-[54px]'>{option.formatedPrice}</Text>
									<View className='flex flex-col gap-0.5 flex-1'>
