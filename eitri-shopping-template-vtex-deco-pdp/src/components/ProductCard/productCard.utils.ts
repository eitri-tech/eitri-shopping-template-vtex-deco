...
export const getProductVideo = (product?: VtexProduct): string => {
	// App.configs stub doesn't declare appConfigs — same library typing gap noted elsewhere.
...
export const calculateBadge = (product?: VtexProduct, sellerDefault?: VtexSeller): string[] => {
	...
	if (
		(sellerDefault?.commertialOffer as { teasers?: Array<{ name?: string }> } | undefined)?.teasers?.some(teaser =>
