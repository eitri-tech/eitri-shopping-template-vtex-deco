...
export default function SubscriptionHistoryModal(props) {
	...
	useEffect(() => {
		...
		listSubscriptionCycles(subscriptionId)
			.then(result => setCycles([...(result || [])].sort((a, b) => new Date(b.date) - new Date(a.date))))
					...
					cycles.map((cycle, index) => (
						<View
							...
							<View className='flex flex-col pb-4'>
								<Text className='text-sm text-gray-900'>{cycleLabel(cycle, t)}</Text>
