...
interface GroupSla extends VtexSla {
...
export default function productGroupShippingResolver(cart: VtexCart): EnrichedProductGroup[] | null {
	try {
 ...
 */
function createSlaKey(slas: VtexSla[]): string {
	return slas
		.map(sla => sla.id)
		.sort()
		.join('|')
 ...
 */
function groupProductsBySlas(
	logisticsInfo: VtexLogisticsInfo[],
	cartItems: VtexCart['items'],
	cart: VtexCart
): ProductGroup[] {
					...
					deliveryAddress:
						sla.deliveryChannel === 'delivery'
							? (cart?.shippingData?.availableAddresses?.find(
									address =>
										address.addressType === 'residential' && address.addressId === logisticItem.addressId
								) ?? null)
							: null,
					totalPrice: 0
				})),
 ...
 */
function enrichShippingGroups(groups: ProductGroup[], cart: VtexCart): EnrichedProductGroup[] {
