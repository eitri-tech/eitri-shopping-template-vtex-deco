import type { VtexSla } from '../types/vtex'

export const formatAmountInCents = (amount: unknown): string => {
...
export const addHoursToDate = (hoursToAdd: number): Date => {
	const currentDate = new Date()
	currentDate.setHours(currentDate.getHours() + hoursToAdd)
	return currentDate
}

export const addMinutesToDate = (minutesToAdd: number): Date => {
	const currentDate = new Date()
	currentDate.setMinutes(currentDate.getMinutes() + minutesToAdd)
	return currentDate
}

export const formatShippingEstimate = (sla: VtexSla): string => {
	// shippingEstimate can come missing from VTEX for some delivery channels; without this
	// fallback every .indexOf() below throws on a plain product page.
...
export const getShippingEstimate = (sla: VtexSla): Date => {
	const shippingEstimate = sla.shippingEstimate ?? ''
