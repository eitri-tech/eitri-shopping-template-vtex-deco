...
interface TrackableSku {
	itemId?: string
	nameComplete?: string
	name?: string
	price?: number | string
	sellers?: Array<{ Price?: number; commertialOffer?: { Price?: number } }>
	variations?: Array<{ values?: string[] }>
	quantity?: number
	[key: string]: unknown
}
...
export default class TrackingService_old {
	 ...
	 *
	 * @example
	 * await TrackingService_old.sendEitriTracking('add_to_cart', { item_name: 'Camiseta', quantity: 2 });
	 */
	static sendEitriTracking = async (event: string, data: Record<string, unknown>, logEvent = true): Promise<void> => {
		let params = {
	 ...
	 *
	 * @example
	 * await TrackingService_old.sendEitriCrashLog('login', { message: 'Erro ao autenticar usuário', stack: '...' });
	 */
	static sendEitriCrashLog = async (event: string, error: Record<string, unknown>): Promise<void> => {
		let params = {
	 ...
	 *
	 * @example
	 * await TrackingService_old.screenView('Home', 'Home');
	 * await TrackingService_old.screenView('Carrinho', 'CartScreen', true);
	 */
	static screenView = async (friendlyScreenName: string, screenFilename?: string, logEvent = false): Promise<void> => {
		try {
	 ...
	 */
	static search = async (term: string, category = '', totalResults: number | string = ''): Promise<void> => {
		try {
	 ...
	 * @returns {Promise<void>} - Promessa resolvida após o envio do erro ao sistema de tracking.
	 */
	static event = async (eventName = '', data: Record<string, unknown> = {}, logEvent = false): Promise<void> => {
		try {
	 ...
	 *
	 * @example
	 * await TrackingService_old.product(produto);
	 * await TrackingService_old.product(produto, 'USD');
	 */
	static product = async (product: VtexProduct, currency = 'BRL'): Promise<void> => {
		try {
	 ...
	 *
	 * @example
	 * const items = TrackingService_old.mountProductItem(produto, 2, 'shelf-1', 'Mais Vendidos');
	 */
	static mountProductItem = (
		product: TrackableProduct,
		quantity?: number,
		itemListId?: string,
		itemListName?: string
	): TrackingItem[] => {
		const _item: TrackableSku = Array.isArray(product?.items) ? product.items[0] : (product as TrackableSku)

		const _categories = product?.categories && product.categories.length > 0 ? product.categories[0] : ''
		const categories = (_categories?.split('/')?.filter(Boolean) ?? []).reduce<Record<string, string>>(
			(acc, curr, index) => {
				if (index === 0) {
		...
		let price: number | string = _item?.price || 0
		if (Array.isArray(_item?.sellers) && _item?.sellers[0]?.Price) {
			price = _item.sellers[0].Price as number
		} else if (Array.isArray(_item?.sellers) && _item?.sellers[0]?.commertialOffer?.Price) {
			price = _item.sellers[0].commertialOffer!.Price as number
		}
		...
		const items: TrackingItem[] = [
			{
				...
				...categories,
				price: quantity ? Number(price) * quantity : price,
				...(quantity && { quantity }),
	 ...
	 *
	 * @example
	 * TrackingService_old.addItemToCart(produto, 2);
	 * TrackingService_old.addItemToCart(produto, 1, 'USD');
	 */
	static addItemToCart = (product: TrackableProduct, quantity = 1, currency = 'BRL'): void => {
		try {
	 ...
	 *
	 * @example
	 * TrackingService_old.removeItemFromCart(produto, 1);
	 * TrackingService_old.removeItemFromCart(produto, 2, 'USD');
	 */
	static removeItemFromCart = (product: TrackableProduct, quantity = 1, currency = 'BRL'): void => {
		try {
			const items = this.mountProductItem(product, quantity)
	 ...
	 *
	 * @example
	 * TrackingService_old.viewCart(carrinho);
	 * TrackingService_old.viewCart(carrinho, 'USD');
	 */
	static viewCart = (cart: VtexCart, currency = 'BRL'): void => {
		try {
			const items = cart?.items.map(item => {
				// Mutating item.price in place (turning the numeric cents value into a formatted
				// string) touched the shared cart object — any code reading `cart` after this
				// tracking call would see the wrong type. Format a clone instead.
				const formattedItem = { ...item, price: ((item.price ?? 0) / 100).toFixed(2) }
				const _item = this.mountProductItem(formattedItem, formattedItem?.quantity || 1)
				return _item
			})
	 ...
	 *
	 * @example
	 * await TrackingService_old.inngageEvent('compra_realizada', { valor: 100 });
	 * await TrackingService_old.inngageEvent('login', { userId: 123 }, true);
	 */
	static inngageEvent = async (eventName: string, data: Record<string, unknown>, logEvent = false): Promise<void> => {
		try {
	 ...
	 *
	 * @example
	 * await TrackingService_old.appsFlyerEvent('compra_realizada', { valor: 100 });
	 * await TrackingService_old.appsFlyerEvent('login', { userId: 123 }, true);
	 */
	static appsFlyerEvent = async (eventName: string, data: Record<string, unknown>, logEvent = false): Promise<void> => {
		try {
