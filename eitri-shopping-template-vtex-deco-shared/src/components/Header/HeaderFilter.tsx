...
interface HeaderFilterProps {
	...
	facetsModalReady?: boolean
	hasFilters?: boolean
	iconColor?: string
}

export default function HeaderFilter(props: HeaderFilterProps) {
	const { handleFilterModal, facetsModalReady, hasFilters, iconColor } = props

	return (
		<View className='relative'>
			<View className='cursor-pointer hover:opacity-80 transition-opacity'>
				<svg
					viewBox='0 0 24 24'
					fill='none'
			...
			{hasFilters && <View className='absolute -right-1 -top-0.5 w-3 h-3 bg-primary-700 rounded-full' />}
		</View>
	)
}
