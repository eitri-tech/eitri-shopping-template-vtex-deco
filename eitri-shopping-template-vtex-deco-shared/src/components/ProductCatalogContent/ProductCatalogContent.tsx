...
export default function ProductCatalogContent(props: Props) {
	...
	const [siblingsByCode, setSiblingsByCode] = useState<Record<string, any[]>>({})
	...
	const getProducts = async (selectedFacets: any, page: number) => {
		try {
			...
			const loadedProducts =
				page === 1 ? result.products.length : products.length + result.products.length
	...
	const loadSiblings = async (pageProducts: any[]) => {
		try {
			const codes = [...new Set(pageProducts.map(getAgrupadorCode).filter(Boolean))] as string[]
						...
						<View className='px-4 pt-5 pb-3'>
							<Text className='text-base text-black'>
								{t(
									title
										? totalProducts === 1
											? 'productCatalog.productCountSingle'
											: 'productCatalog.productCountMultiple'
										: totalProducts === 1
											? 'productCatalog.resultCountSingle'
											: 'productCatalog.resultCountMultiple',
									{ count: totalProducts, title: title || t('productCatalog.title') }
								)}
							</Text>
						</View>
					)}
