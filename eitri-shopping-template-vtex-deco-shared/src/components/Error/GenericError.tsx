...
export default function GenericError(props: GenericErrorProps) {
	...
	async function getConfigs() {
		try {
			const configs = (await Eitri.getConfigs()) as { miniAppData?: { slug?: string } }
		...
		<View className='flex items-center justify-center min-h-[85vh] bg-opacity-90'>
			<View className='flex flex-col w-full h-full max-w-md rounded-2xl'>
				...
				<View className='flex flex-col h-full p-4'>
					<Text className='font-bold text-[23px] mt-5 mb-4 text-center'>Não foi possível continuar</Text>
