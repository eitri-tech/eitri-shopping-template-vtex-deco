...
export interface Props {
	...
	title?: string
	/**
	 * @title Conteúdo (Draft.js — string JSON ou objeto).
	 */
	content?: string | DraftContent
}

const parseContent = (raw?: string | DraftContent): DraftContent | null => {
	if (!raw) return null
...
const getSegments = (
	...
	return sorted.slice(0, -1).reduce<Segment[]>((acc, start, i) => {
		...
		const cls = (ranges ?? [])
			...
			.map(r => (r.style ? STYLE_CLASS[r.style] ?? '' : ''))
			.filter(Boolean)
			.join(' ')

		const entityRange = (entityRanges ?? []).find(e => e.offset <= start && start < e.offset + e.length)
		const entity = entityRange?.key != null ? entityMap?.[entityRange.key] : null
...
function InlineText({ text, ranges, entityRanges, entityMap, base }: InlineTextProps) {
...
const renderBlock = (block: DraftBlock, index: number, blocks: DraftBlock[], entityMap?: Record<string, any>) => {
	const { text, type, inlineStyleRanges: ranges, entityRanges, depth = 0 } = block
	const indent = depth > 0 ? `pl-${depth * 6}` : ''

	const inline = (base: string) => (
		<InlineText
			key={index}
			text={text}
			ranges={ranges}
...
export default function RichText({ title, content: rawContent }: Props) {
	const content = parseContent(rawContent)

	if (!content?.blocks?.length && !title) return null
