...
```
npx -y -p typescript@6 tsc --noEmit -p tsconfig.json
```

Pin `typescript@6` explicitly -- `npx tsc` with no version pin can resolve to whatever `npx` finds first (including a much newer major, e.g. TypeScript 7's Go-based rewrite, which has different module-resolution defaults and produces hundreds of bogus `"has no exported member"` errors against this project's path-mapped `eitri-luminus` types). If a previous session left a `package.json`/`node_modules` behind (e.g. from installing `typescript` as a real dependency to work around `npx` resolving to a decoy), delete them rather than trying to reconcile a lockfile -- the `npx -p typescript@6` invocation above is the one and only supported way to type-check this repo. TypeScript 6 is stricter than 5 (more implicit-any and null/undefined-narrowing errors surface) -- codebase is clean under it as of this pin.
...
- **Casting around library `.d.ts` gaps:** when a Luminus component's `.d.ts` is provably tighter than real, pre-existing usage (a prop it doesn't declare, like `View`'s missing `backgroundColor`, or legacy props from an older API version), preserve the exact runtime behavior with a local override rather than deleting the prop or rewriting the component:
  ...
  **Before creating one of these casts (or touching an existing one), check whether the prop is actually a no-op — don't assume "undeclared in the `.d.ts`" means "works at runtime, just untyped."** The check differs by where the target component is defined:
  - **Local component (authored in this repo, e.g. a wrapper under `src/components/`):** read its source directly. If it destructures only specific props and never spreads `...rest` onto what it renders, any prop the caller passes beyond that list is dropped with zero effect — a real no-op, not a typing gap. Found this exact case in `ProductCardLoading.tsx` (pdp) and `ProductCardDefault.tsx` (shared): both cast `Loading` (the shared `LoadingComponent.tsx` wrapper) to pass `width`/`inline`, but `LoadingComponent` only ever reads `isLoading`/`fullScreen` — those props did nothing. Fix: drop the no-op prop from the call site, and drop the cast entirely if nothing else on that call needs it (use the plain, correctly-typed import instead).
  - **`eitri-luminus` primitive (`View`, `Text`, `TextInput`, `Lottie`, `Page`, ...):** you generally can't read the installed package's real implementation from a fresh checkout — it only exists in a per-machine cache at `~/.eitri/<app-id>/node_modules/...` after `eitri app start` has run at least once on that machine (see "Ground truth for tricky runtime behavior" below for the path pattern). If that cache isn't present, don't assume either way; keep the existing `// likely a no-op` / `// legacy prop from an older API version` comment as-is rather than deleting a cast you can't verify. Only remove the cast once you've actually confirmed (via the installed package, or a device test showing the prop has no visible effect) that it's dead.
