...
export default function AddCardForm() {
	...
	const [systemGroup, setSystemGroups] = useState<PaymentSystemGroup | undefined>(undefined)
	const [isLoading, setIsLoading] = useState(false)
	const [formCardInfo, setFormCardInfo] = useState<CheckoutCardInfo | null>(null)

	const [validCard, setValidCard] = useState(false)
	const [validDueDate, setValidDueDate] = useState(false)

	useEffect(() => {
		TrackingService.sendScreenView('Adicionar cartão', 'AddCardForm')
	}, [])
	...
	const setPaymentSystem = async () => {
		try {
			setIsLoading(true)
			const paymentSystem = formCardInfo?.cardNumber ? findPaymentSystem(formCardInfo.cardNumber) : undefined
			if (paymentSystem && cart) {
				const payload = {
					payments: [
						{
	...
	const handleCardDataChange = (key: keyof CheckoutCardInfo, e: ChangeEvent<HTMLInputElement>) => {
