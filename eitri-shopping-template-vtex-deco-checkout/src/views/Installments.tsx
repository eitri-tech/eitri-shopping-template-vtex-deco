...
interface InstallmentsState {
	paymentSystem?: ResolvedPaymentSystem
	description?: string
}

// Raw entry from VtexInstallmentOption.installments (types/vtex.ts) — no `interestRate` field on the
// real VTEX payload, but existing code reads it defensively (`?? 0`), so it's kept optional here too.
...
export default function Installments(props: RouteProps<InstallmentsState>) {
	...
	const handleInstallmentSelect = async (installment: RawInstallment) => {
		try {
			setIsLoading(true)
						...
						return (
							<View
								...
								<View className='flex flex-col'>
									...
									{(installment.count ?? 0) > 1 && (
										<Text className='text-sm text-neutral-500'>
											{installment.hasInterestRate
												? t('installments.txtWithInterest')
												: t('installments.txtNoInterest')}
										</Text>
									)}
								</View>
								<View className='flex flex-row items-center gap-2'>
									{(installment.count ?? 0) > 1 && (
										<Text className='text-sm text-neutral-600'>{formatAmountInCents(installment.total)}</Text>
									)}
